...
public class ApplicationRunner {
    ...
    public ApplicationRunner(IOServiceStreams ioService, QuestionGeneratorService generatorService,
                             AnswerAnalyzerService answerAnalyzerService, MessageSource messageSource, ApplicationConfig applicationConfig) {
    ...
    public void startTesting() {
        ...
        for (int i = 0; i < totalQuestions; i++) {
            ...
            ioService.outputString(
                    getStringByLocal("TheAnswerIs", null)
    ...
    private void outPutMyLanguage() {
        for (int i = 1; i <= localeList.size(); i++) {
            locale = localeList.get(i - 1);
            ioService.outputString(i + " " + getStringByLocal("MyLanguage", null));
        }
    }
    ...
    private void printTestResult() {
        ...
        ioService.outputString(lastFirstName + " " + getStringByLocal("Result", null) + " "
                + (answerAnalyzerService.isTestPassed() ? "" : getStringByLocal("TestNot", null)) + getStringByLocal("passed", null));
    }
