...
public class RequestedDocuments {
    ...
    @Comment("Версия API. Текущая версия: v1. Для документа (Сведения о состоянии индивидуального страхового счета застрахованного лица) (ILS_PFR) доступна вторая версия (v2) API")
    ...
    @Comment("Признак расширенной модели данных (может быть указан только в отношении свидетельств о браке и разводе). Если указан extended=true, то тогда в ответе вернется информация о соответствующей записи акта гражданского состояния (при наличии)")
