...
public class BookServiceImpl implements BookService {
    ...
    private void deleteComments(Book book) {
        for (BookComment bookComment : book.getComments()) {
            if (bookComment != null) {
                Optional<BookComment> optional = bookCommentRepository.findById(bookComment.getId());
        ...
        for (Author author : book.getAuthors()) {
            if (author != null) {
                ...
                if (optional.isPresent()) {
                    optional.get().getBooks().remove(book);
    ...
    private void deleteLinkGenres(Book book) {
        for (Genre genre : book.getGenres()) {
            if (genre != null) {
                Optional<Genre> optional = genreRepository.findById(genre.getId());
                if (optional.isPresent()) {
                    optional.get().getBooks().remove(book);
