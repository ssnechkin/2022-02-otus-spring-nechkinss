...
public class BookController implements MenuItems {
    ...
    public Content edit(@PathVariable("id") long id) {
        ...
        save.setLink(new Link(HttpMethod.PUT, "/book/" + book.getId()));
    ...
    private Table getTableAll() {
        ...
        for (Book book : service.getAll()) {
            Row row = new Row();
            row.setLink(new Link(HttpMethod.GET, "/book/" + book.getId()));
    ...
    private Content getContentView(long id) {
        Book book = service.getById(id);
        Content content = new Content();
        Button edit = new Button(), delete = new Button(), comments = new Button(), mAuthors = new Button(), mGenres = new Button();
        Field name = new Field(), authors = new Field(), genres = new Field();
        content.setPageName("Книга");
        edit.setPosition(1);
        edit.setTitle("Редактировать");
        edit.setLink(new Link(HttpMethod.GET, "/book/edit/" + book.getId()));
        delete.setPosition(2);
        delete.setTitle("Удалить");
        delete.setLink(new Link(HttpMethod.DELETE, "/book/" + book.getId()));
        comments.setPosition(2);
        comments.setTitle("Комментарии");
        comments.setLink(new Link(HttpMethod.GET, "/book/" + book.getId() + "/comments"));
        mAuthors.setPosition(2);
        mAuthors.setTitle("Авторы");
        mAuthors.setLink(new Link(HttpMethod.GET, "/book/" + book.getId() + "/authors"));
        mGenres.setPosition(2);
        mGenres.setTitle("Жанры");
        mGenres.setLink(new Link(HttpMethod.GET, "/book/" + book.getId() + "/genres"));
